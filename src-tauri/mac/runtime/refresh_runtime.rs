...
fn try_refresh_via_chatgpt_api(
    ...
    // alongside the now-correct Free plan label. `unwrap_or_default()`
    // turns the `None` case into an empty `QuotaSummary`, which is
    // semantically correct (no quota = no allotment).
    sync_profile_quota(
        profile_name,
...
fn refresh_via_app_server(
    ...
    // app-server rotates the per-profile `auth.json` inside the sandbox
    // when `account/read` runs with `refreshToken: true`. Copy it back so
    // the user's profile picks up fresh tokens, mirroring the prior
    // copy-back step from the legacy `codex exec` flow.
    ...
    let now_ms = std::time::SystemTime::now()
        ...
        .ok()
        .and_then(|value| u64::try_from(value.as_millis()).ok());
    // Always write the quota snapshot, including the empty default for
    // accounts whose payload had no rate-limit data (free tier
    // downgrade): keeps stale paid-window numbers from sticking next to
    // a freshly-updated plan label.
    sync_profile_quota(
        profile_name,
...
mod tests {
    ...
    use crate::macos::cli_shim::get_refresh_runtime_dir;
    use std::fs;
    ...
    fn prepare_refresh_runtime_home_preserves_existing_config_and_prunes_extra_features() {
        ...
        assert_eq!(runtime_home, get_refresh_runtime_dir(&codex_home));
        assert_eq!(
